...
def window_partition(x: torch.Tensor,
                     window_size: Sequence[int]) -> torch.Tensor:  # 返回类型
    """
    Args:
    ...
    """
    B, D, H, W, C = x.shape  # Batch size，维度，长度，宽度，通道数
...
def compute_mask(D: int, H: int, W: int, window_size: Sequence[int],
    ...
    cnt = 0
    for d in slice(-window_size[0]), slice(-window_size[0], -shift_size[0]), slice(-shift_size[0], None):  # 切片
...
class WindowAttention3D(BaseModule):
    ...
    def __init__(self,
        ...
        self.scale = qk_scale or head_dim ** -0.5  # 根号d分之1
        ...
        coords_d = torch.arange(self.window_size[0])  # [0,1,2,...,self.window_size[0]-1]
        ...
        # [3, Wd*Wh*Ww, 1] - [3, 1, Wd*Wh*Ww]
        relative_coords = coords_flatten[:, :, None] - coords_flatten[:, None, :]  # 3, Wd*Wh*Ww, Wd*Wh*Ww
        # Wd*Wh*Ww, Wd*Wh*Ww, 3
        relative_coords = relative_coords.permute(1, 2, 0).contiguous()
        # shift to start from 0
        relative_coords[:, :, 0] += self.window_size[0] - 1  # 三元索引变一元
    ...
    def forward(self,
        ...
        q = q * self.scale
        # [batch_size*num_windows, num_heads, Wd*Wh*Ww, Wd*Wh*Ww ]
        attn = q @ k.transpose(-2, -1)  # 转置

        # [Wd*Wh*Ww*Wd*Wh*Ww,nH]->[Wd*Wh*Ww,Wd*Wh*Ww,nH]
        relative_position_bias = self.relative_position_bias_table[
            self.relative_position_index[:N, :N].reshape(-1)].reshape(
            N, N, -1)  # Wd*Wh*Ww,Wd*Wh*Ww,nH 根据index从relative_position_bias_table取参数
        ...
        if mask is not None:
            nW = mask.shape[0]
            # attn.view:[batchsize, num_windows, num_heads, Wd*Wh*Ww, Wd*Wh*Ww]
            # mask.unsqueeze:[1, nW ,1 , Wd*Wh*Ww, Wd*Wh*Ww]
...
class VideoSwinCLSTMTransformerBlock(BaseModule):
    ...
    def __init__(self,
        ...
        # mlp前加入3DLSTM
        # Embedding the ConvLSTM module only in Stage2 and Stage4
        if embed_dims == 192 or embed_dims == 768:
            self.conv_lstm = ConvLSTM(self.embed_dims, self.embed_dims, kernel_size=(3, 3), num_layers=1,
                                      batch_first=True)
    ...
    def forward_part1(self, x: torch.Tensor,
        ...
        if pad_d1 > 0 or pad_r > 0 or pad_b > 0:
            x = x[:, :D, :H, :W, :].contiguous()  # 把前面pad的数据移除掉
        return x
    ...
    def forward(self, x: torch.Tensor,
        ...
        if x.size(-1) == 192 or x.size(-1) == 768:
            # 加入ConvLSTM
...
class PatchMerging(BaseModule):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if pad_input:
            x = F.pad(x, (0, 0, 0, W % 2, 0, H % 2))  # W % 2表示填充几行
...
class BasicLayer(BaseModule):
    ...
    def __init__(self,
        ...
        for i in range(depth):
            ...
            block = VideoSwinCLSTMTransformerBlock(**_block_cfg)
            self.blocks.append(block)

        # if self.embed_dims == 768:
        #     self.conv_lstm = ConvLSTM(self.embed_dims, self.embed_dims, kernel_size=(3, 3), num_layers=2,
        #                               batch_first=True)
...
class VideoSwinCLSTMTransformer(BaseModule):
    """Video Swin-CLSTM Transformer backbone.
    ...
    def __init__(
        ...
        # Initialize SparseTubesTokenizer-related parameters
        hidden_dim = self.embed_dims  # You can adjust based on your needs
        kernel_sizes = (
            (8, 8, 8),
            (16, 4, 4),
            (4, 12, 12),
        ...
        if self.out_after_downsample:
            self.num_features = embed_dims[1:]  # 从第二个元素开始到最后一个元素的子列表
        else:
            self.num_features = embed_dims[:-1]  # 用于获取列表 embed_dims 中从第一个元素开始到倒数第二个元素的子列表
    ...
    def forward(self, x: torch.Tensor) -> \
        ...
        x = self.patch_embed(x)  # PatchEmbed3D

        x = self.pos_drop(x)  # Dropout

        outs = []
        for i, layer in enumerate(self.layers):
            x = layer(x.contiguous(), do_downsample=self.out_after_downsample)  # BasicLayer 4个
            ...
            if layer.downsample is not None and not self.out_after_downsample:
                x = layer.downsample(x)  # PatchMerging
    ...
    def train(self, mode: bool = True) -> None:
        ...
        super(VideoSwinCLSTMTransformer, self).train(mode)
        self._freeze_stages()
